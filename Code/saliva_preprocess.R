...
pd <- pd[,1:9]
pd$meth_id <- paste0(pd$Sentrix_ID,'_',pd$Sentrix_Position)
...
### GEO sub
setwd('F:/Drive/Saliva_Ref_Panel/Data')

pd <- readRDS('pd.qc.rds')
pd <- readRDS('06-11-19 pd_final_all_samps.rds')
pd <- pd[!pd$celltype %in% 'CD45neg', ]
pd <- pd[pd$Sample.ID != 'SAL19_large',]

samps <- data.frame(Sample_name = pd$Sample.ID,
                    title = paste0('saliva ', pd$celltype, ' DNA from participant ', pd$id),
                    source = 'child saliva',
                    organism = 'Homo sapiens',
                    idat1 = paste0(pd$meth_id,'_Grn.idat'),
                    idat2 = paste0(pd$meth_id,'_Red.idat'),
                    participant = pd$id,
                    sex = ifelse(pd$Male==1, 'male', 'female'),
                    type = pd$celltype,
                    molecule = 'genomic DNA',
                    label = '',
                    description = '',
                    platform = 'GPL21145')
write.csv(samps, file='samples.csv')


#matrix processed

load('minfi-detP.rda')
noob <- readRDS('noob_beta.rds')

identical(colnames(noob),colnames(detP))
identical(rownames(noob),rownames(detP))

noob <- noob[,pd$meth_id]
detP <- detP[,pd$meth_id]

matrix.processed <- matrix(0, ncol=2*ncol(noob)+1, nrow=nrow(noob))
rownames(matrix.processed) <- rownames(noob)
colnames(matrix.processed) <- 1:121
colnames(matrix.processed)[1] <- 'ID_REF'
colnames(matrix.processed)[seq(2,120,2)] <- as.character(pd[match(colnames(noob), pd$meth_id), 'Sample.ID'])
colnames(matrix.processed)[seq(3,121,2)] <- paste0(as.character(pd[match(colnames(noob), pd$meth_id), 'Sample.ID']),'_Detection_PVal')

matrix.processed[,1] <- rownames(matrix.processed)
matrix.processed[,seq(2,120,2)] <- noob
matrix.processed[,seq(3,121,2)] <- detP
matrix.processed[1:5,1:5]

write.table(matrix.processed, sep='\t', file='matrix_processed.txt', row.names=F, quote=F)

setwd('F:/Drive/Saliva_Ref_Panel/GEO/ImageData')
files <- list.files(recursive=T)
included_files <- files[grepl(paste(pd$meth_id,collapse='|'), files)]

file.copy(from=included_files,
          to=gsub('.*/','',included_files))
