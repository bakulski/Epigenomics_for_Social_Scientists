...
graphColors = c("#023FA5","#7D87B9","#BEC1D4","#D6BCC0","#BB7784", "#D33F6A", "#11C638","#8DD593","#C6DEC7",
...
#Plotpcs function is just application of legend and pairs functions with some convenient changes in default arguments
plotpcs <- function(pheno, phenoname, npc = 6){
  pheno <- as.factor(pheno)
  mycols <- myColors
  if(length(levels(pheno)) > length(myColors)){
    mycols <- graphColors
  }
  
  plot.new()
  legend("bottom", levels = pheno, fill = mycols, title = sprintf("Principal Components by %s", phenoname))
  pairs(prin$x[, 1:npc], col = pheno, labels = paste0("PC", 1:npc), pch = 1, cex = 0.5)
}

#Updatable list of phenotype name and its location 
phenos <- c(
  "Sex"            = pd$gender,
  "Batch"          = pd$Batch,
  "Array Position" = pd$Array,
  "Slide"          = pd$Slide
) 

#Raw PCA plots
pdf(file.path(output_dir,"PCA-plots-Raw.pdf"))

screeplot(prin, col = "dodgerblue", xlab = "Principal Components of Raw Beta Values", main = "", cex.lab = 1.3)

par(mar = c(0, 0, 0, 0))
for(p in phenos){
  print(plotpcs(pheno = p, phenoname = names(p)))
}
