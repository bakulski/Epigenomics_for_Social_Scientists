...
mod <- model.matrix( ~ factor( pd.complete$casestatus ) + pd.complete$age + factor( pd.complete$gender ) + factor( pd.complete$smoking ) + pd.complete$cellscores$x[ ,1 ] )  
```
...
fit.adj <- lm(pd.complete$meanDNAm ~ factor( pd.complete$casestatus ) + pd.complete$age + factor( pd.complete$gender ) + factor( pd.complete$smoking ) + pd.complete$cellscores$x[ ,1 ])
...
# Plot meQTL
```{r plot meQTL}
pdf( paste0(output_dir, "VolcanomeQTL.pdf" ))
